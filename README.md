...
`vulcanary dataflow-prototype PATH --json dataflow.json` runs a separate research-only Python code-injection analysis. It follows recognized request input to `eval` and `exec` across supported same-module calls and explicit repository-local imports, and defaults to a maximum call depth of three. The report surfaces depth truncations, parse failures, and unresolved call returns that flow into a sink as distinct analysis gaps. Missing modules and imported symbols that cannot be resolved are categorized separately; calls through receivers whose type is unknown remain `dynamic_dispatch`; plain unknown calls are `unresolved_call`; and bounded exits distinguish `depth_limit` from `recursion_cycle`. Trivially static local returns are proven clean rather than counted as gaps. Other unsupported Python constructs may still be absent from these counters. Sink locations—not movable source locations—anchor fingerprints. Recognized conversions and parsers are retained as sanitizer evidence and lower confidence; they never silently terminate propagation.

Analysis is also bounded independently by module count, evaluated call count, and wall time. Defaults are 10,000 Python modules, 1,000,000 evaluated calls, and 120 seconds; use `--max-modules`, `--max-calls`, and `--timeout-seconds` to lower or raise them. The current 2,222-module external runtime corpus requires 676,664 evaluated calls, leaving about 48% call-budget headroom; it completes within the time budget without a global limit. The report records the configured budget, observed module and call counts, and every exhausted limit under `analysis_limits`. Reaching a limit produces an incomplete-analysis signal rather than a clean result.

Time-limit records include the elapsed seconds observed when analysis stopped, so operators can calibrate the budget from evidence instead of an unspecified timeout marker.
...
Nested functions at any lexical depth that directly capture known outer taint and contain an `eval` or `exec` sink are reported as `unsupported_closure` gaps. Vulcanary does not currently propagate closure cells or assignments through intermediate closure scopes. Clean captures and bindings shadowed directly inside the sink-owning function are proven clean; a value shadowed only in an intermediate closure scope may still be conservatively gapped. On the external runtime corpus this adds five explicit gaps with no exposure, workload, truncation, or global-limit change. Empty-taint merge fast paths preserve the exact exposure, gap, truncation, and analyzed-call identity sets while reducing a measured full-corpus run from the recent 88–97 second range to 64.7 seconds on the same machine; runtime remains environment-dependent.

Pass BenchmarkPython's `expectedresults-0.1.csv` with `--benchmark-expected` to add true/false-positive counts, recall, precision, false-positive rate, and the benchmark's TPR−FPR score. Against BenchmarkPython commit `f1291485808b66e20ddb6b01b10dc71b3df8c8ba`, the initial bounded prototype found 1/20 true CWE-94 cases and 3/33 false cases (5% recall, 25% precision, 9.1% false-positive rate, TPR−FPR −0.041). Key-sensitive ConfigParser state, recognized request-wrapper sources, common encoding/decoding transforms, and conservative container propagation first improved that result to 7/20 true cases and 7/33 false cases. Visiting augmented assignments, propagating their values, modeling loop and comprehension targets, and recognizing raw query strings now finds 18/20 true cases and 17/33 false cases (90% recall, 51.4% precision, 51.5% false-positive rate, TPR−FPR +0.385). The two remaining true cases use `match`/`case` and are explicitly gapped; a path-insensitive implementation reached 100% recall but reduced precision to 45.5% and TPR−FPR to +0.273, so it was rejected. Unsupported named expressions, unpacking, match selection, and complex augmented-assignment or loop targets emit explicit gaps rather than silently disappearing. The broader propagation produced zero exposures across a 3,388-file Python standard-library and site-packages corpus, but the BenchmarkPython false-positive rate remains evidence that this research prototype is not ready to gate releases. Source-producing return values now resolve through bounded same-module and repository-local imported function chains, including aliases and package-relative imports, even when the helper receives no tainted argument. Calls on directly constructed repository-local classes also resolve across local, imported-symbol, and module-alias forms without importing or executing scanned code. Constructor-derived instance attributes are carried into later method calls, including nested objects such as `self.reader`, and tainted values assigned to instance state remain visible to subsequent calls on that analyzed object. Local aliases of those instances and nested objects share allocation identity, so a mutation through an alias is visible through the original receiver without conflating separate allocations of the same class. Single-inheritance lookup now follows repository-local base constructors and instance methods across module boundaries; child overrides take precedence. Zero-argument `super().method()` return values resolve from the current lexical class through the same bounded parent lookup. Cross-module inherited attribute writes are marked `cross_module_inherited_state` when they reach a sink because continuity across separate child method calls is not yet a complete guarantee. Methods that return receiver attributes are treated as state-relevant, allowing repository-local inherited writers to propagate through the returned value into an external sink; unresolved inherited state remains explicitly gapped. Explicit-argument `super`, multiple inheritance, unresolved external bases, and inheritance cycles remain reported gaps rather than guessed paths. Cached clean constructor summaries receive fresh allocation identities on reuse and preserve internal alias relationships; unresolved or truncated summaries are not cached. Corpus comparisons report set-level additions and removals for gaps and truncations, so lower workload cannot masquerade as broader coverage. This is bounded path-local modeling, not heap analysis: descriptors, framework injection, container-held aliases, multiple-inheritance method resolution, and receiver types that cannot be proven from a repository-local constructor remain explicit gaps. Missing modules, unresolved imported symbols, depth limits, and recursion cycles are also surfaced. Updated external-corpus metrics must be recorded before this capability is released. These deliberately published weak baselines prevent fixture-only confidence. BenchmarkPython v0.1 is preliminary; scores are directional, not a product-performance claim.
...
- **Dependency results require resolved versions.** Manifest ranges are reported as unresolved rather than guessed. Maven requires committed dependency-tree JSON, Gradle requires a lockfile, NuGet requires `packages.lock.json`, and CycloneDX is the escape hatch for other ecosystems. Invalid or unreadable recognized manifests and declared npm, Cargo, Composer, or Bundler dependencies without a governing lockfile produce an explicit incomplete-analysis record and `dependency: gap` coverage state; they cannot silently look clean. Legitimately empty manifests remain clean. Vulcanary does not run package-manager resolution during ordinary scans.
...
Maven inventory reads committed JSON output from the Maven Dependency Plugin named `dependency-tree.json` or `maven-dependency-tree.json`. Generate it with a current `maven-dependency-plugin` using `dependency:tree -DoutputType=json -DoutputFile=dependency-tree.json`. Vulcanary preserves the resolved tree's directness and scope but never evaluates POM inheritance or runs Maven. Gradle inventory reads resolved `group:name:version=configurations` entries from `gradle.lockfile`; because the lock does not identify declaration depth, Gradle packages remain conservatively transitive/read-only. A `pom.xml` or Gradle build file without its corresponding resolved artifact produces a scanner-health coverage warning rather than a clean dependency result.
...
Reachability correlation is deliberately evidence-only. Java/Kotlin imports are correlated to Maven group namespaces; C# `using` directives to NuGet package namespaces; Go imports to module-path prefixes; Rust `use`/`extern crate` roots to normalized crate names; and Ruby `require` roots to gem names. Maven evidence is group-level rather than artifact-level because artifact IDs do not appear in Java imports, so sibling artifacts in one group can share the same evidence. Package/import naming differences such as `com.google.guava:guava` versus `com.google.common.*`, or `activesupport` versus `active_support`, can also leave genuine use unobserved. These signals improve remediation priority but never lower advisory severity or claim the vulnerable code path executes; their absence is not evidence of safety. Dynamic loading, dependency aliases, generated code, and ecosystems whose package names do not map cleanly to source namespaces can remain unknown.
