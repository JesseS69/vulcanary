...
Nested functions that capture known taint and contain an `eval` or `exec` sink are reported as `unsupported_closure` gaps. Vulcanary does not currently propagate closure cells; clean captured values do not produce this gap. On the external runtime corpus this adds five explicit gaps with no exposure, workload, truncation, or global-limit change.
