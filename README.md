...
Reachability correlation is deliberately evidence-only. Java/Kotlin imports are correlated to Maven group namespaces; C# `using` directives to NuGet package namespaces; Go imports to module-path prefixes; Rust `use`/`extern crate` roots to normalized crate names; and Ruby `require` roots to gem names. These signals improve remediation priority but never lower advisory severity or claim the vulnerable code path executes. Dynamic loading, dependency aliases, generated code, and ecosystems whose package names do not map cleanly to source namespaces can remain unknown.
