...
Nested functions at any lexical depth that directly capture known outer taint and contain an `eval` or `exec` sink are reported as `unsupported_closure` gaps. Vulcanary does not currently propagate closure cells or assignments through intermediate closure scopes; clean and locally shadowed captured values do not produce this gap. On the external runtime corpus this adds five explicit gaps with no exposure, workload, truncation, or global-limit change. Empty-taint merge fast paths preserve the exact exposure, gap, truncation, and analyzed-call identity sets while reducing a measured full-corpus run from the recent 88–97 second range to 64.7 seconds on the same machine; runtime remains environment-dependent.
