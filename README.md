...
The dashboard's **Verification receipts** panel validates stored proof hashes on every load and marks altered records invalid. Individual receipts can be downloaded as normalized JSON; downloads contain sanitized verification metadata but never raw command output, environment values, credentials, or source content.
